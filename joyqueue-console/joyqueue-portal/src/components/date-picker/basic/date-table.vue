<template>
  <table
    ...
    <tbody>
    <tr>
      <th v-if="showWeekNumber">{{ t(localePrefix + 'week') }}</th>
      <th v-for="(week, key) in WEEKS" :key="key">{{ t(localePrefix + 'weeks.' + week) }}</th>
    </tr>
    <tr
      v-for="(row, key) in rows"
      :class="[prefixCls + '__row', { current: isWeekActive(row[1]) }]"
      :key="key">
      <td
...
import { hasClass, arrayFindIndex, arrayFind, coerceTruthyValueToArray } from '../../../utils/assist'
import Locale from '../../../mixins/locale'

const prefixCls = `${Config.clsPrefix}date-table`
  ...
  computed: {
    tableClasses () {
      return [
        `${prefixCls}`,
        {
          'is-week-mode': this.selectionMode === 'week'
        }
      ]
    },
