<template>
  <div
    ...
    <d-input
      ...
      <Icon slot="prefix"
            :name="triggerClass"
            :class="[clsPrefix + 'input__icon']"
            @click="handleFocus">
      </Icon>
      <Icon slot="suffix"
            :name="showClose ? '' + clearIcon : ''"
            :class="[clsPrefix + 'input__icon']"
  ...
  <div
    ...
    <div :class="innerClasses">
      ...
      <Icon
        ...
        v-if="haveTrigger"
        :name="showClose ? '' + clearIcon : ''"
        :class="[clsPrefix + 'input__icon', clsPrefix + 'range__close-icon']">
      </Icon>
    </div>
    <div v-if="$slots.append" slot="append" :class="[prefixCls + '-group-append']">
      <slot name="append"></slot>
    </div>
  </div>
</template>
...
import { oneOf, merge } from '../../utils/assist'
...
const prefixCls = `${clsPrefix}date-editor`
  ...
  computed: {
    ...
    innerClasses () {
      return [
        ...
        `${prefixCls}--` + this.type,
        {
          [`${clsPrefix}range-editor--${this.pickerSize}`]: this.pickerSize,
          [`is-disabled`]: this.pickerDisabled,
          [`is-active`]: this.pickerVisible
        }
      ]
    },
    ...
    triggerClass () {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'clock' : 'calendar')
    },
