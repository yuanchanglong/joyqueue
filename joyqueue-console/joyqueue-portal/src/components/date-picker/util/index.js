...
const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`${localePrefix}weeks.${week}`)),
    dayNames: weeks.map(week => t(`${localePrefix}weeks.${week}`)),
    monthNamesShort: months.map(month => t(`${localePrefix}months.${month}`)),
    monthNames: months.map((month, index) => t(`${localePrefix}month${index + 1}`)),
    amPm: ['am', 'pm']
  }
}
