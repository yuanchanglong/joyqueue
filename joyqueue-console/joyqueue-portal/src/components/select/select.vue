...
import Drop from './dropdown-select.vue'
import {directive as clickOutside} from 'v-click-outside-x'
import TransferDom from '../../directives/transfer-dom'
import { oneOf } from '../../utils/assist'
import { getBoundingClientRect, getToDocL, getToDocT, getWindowW, getWindowH, getDocScrollL, getDocScrollT } from '../../utils/position.js'
...
const optionRegexp = /^d-option$|^Option$/i
...
export default {
  ...
  mounted () {
    ...
    this.dropDownRel = this.$refs.reference
  },
  data () {
    return {
      prefixCls: prefixCls,
  ...
  computed: {
    classes () {
      return [
        `${prefixCls}`,
        {
          ...
          [`${prefixCls}-disabled`]: this.selectDisabled,
          [`${prefixCls}-multiple`]: this.multiple,
          [`${prefixCls}-single`]: !this.multiple,
          [`${prefixCls}-show-clear`]: this.showCloseIcon,
          [`${prefixCls}-${this.selectSize}`]: !!this.selectSize
        }
      ]
    },
    dropdownCls () {
      return {
        ...
        [prefixCls + '-auto-complete']: this.autoComplete,
        [`${prefixCls}-dropdown-multiple`]: this.multiple,
        [`${prefixCls}-dropdown-single`]: !this.multiple
      }
    },
    selectionCls () {
      return {
        ...
        [`${prefixCls}-selection-multiple`]: this.multiple,
        [`${prefixCls}-selection-single`]: !this.multiple
      }
    },
    ...
    localeNotFoundText () {
      if (typeof this.notFoundText === 'undefined') {
        return this.t(localePrefix + 'noMatch')
      } else {
    ...
    localeLoadingText () {
      if (typeof this.loadingText === 'undefined') {
        return this.t(localePrefix + 'loading')
      } else {
    ...
    selectTabindex () {
      return this.selectDisabled || this.filterable ? -1 : 0
    },
    remote () {
  ...
  methods: {
    ...
    hideMenu () {
      ...
      setTimeout(function () { this.unchangedQuery = true }, ANIMATION_TIMEOUT)
    },
    ...
    handleKeydown (e) {
      if (!oneOf(e.key, ['Tab', 'Escape', 'Enter', 'ArrowUp', 'ArrowDown'])) return
      if (this.visible) {
        e.preventDefault()
        if (e.key === 'Tab') {
          e.stopPropagation()
    ...
    dropPrevElRect () {
      let prevEl = this.dropDownRel
      let obj = {}
      if (prevEl) {
        obj.boundingRect = getBoundingClientRect(prevEl)
        obj.toDocTop = getToDocT(prevEl)
        obj.toDocLeft = getToDocL(prevEl)
        obj.windowW = getWindowW()
        obj.windowH = getWindowH()
        obj.scrollLeft = getDocScrollL()
        obj.scrollTop = getDocScrollT()
      }
      return obj
    }
  },
